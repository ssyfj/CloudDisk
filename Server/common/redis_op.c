...
/* -------------------------------------------*/
/**
 * @brief  根据score区间获取相应的元素集合
 *          返回数据为 区间为
 *              [start, end)
 *
 * @param conn
 * @param key       	表名
 * @param start_score   查找表的起始数据下标
 * @param end_score	    查找表的结尾数据下标
 * @param values    	得到表中的value数据
 *      RVALUES为数组指针类型，其变量指向 char[1024]
 *      typedef char (*RVALUES)[VALUES_ID_SIZE];
 * @param get_num   得到结果value的个数
 *
 * @returns
 *      0 succ, -1 fail, 1 还有数据
 */
/* -------------------------------------------*/
int rop_zset_zrange_by_score(redisContext *conn, char* key, long start_score,long end_score,RVALUES values, int *get_num,int max_count)
{
    int retn = 0;
    int i = 0;
    redisReply *reply = NULL;
    ...
    //获取score区间内的所有元素
    reply = redisCommand(conn, "ZRANGEBYSCORE %s %ld %ld", key, start_score, end_score);
    if (reply->type != REDIS_REPLY_ARRAY) //如果返回不是数组
    {
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]ZRANGEBYSCORE %s  error!%s\n", key, conn->errstr);
        retn = -1;
        goto END;
    }
    ...
    //通过element[index] 的方式访问数组元素
    //每个数组元素是一个redisReply对象的指针
    if(reply->elements > max_count)
    {
    	retn = 1;
    }
    end = (reply->elements > max_count) ? max_count: reply->elements;
    *get_num = end; //得到结果value的个数
...
/* -------------------------------------------*/
/**
 * @brief        删除score区间内的所有的成员
 *
 *
 * @param conn       			已建立的链接
 * @param key        			zset表名
 * @param start_score           起始score
 * @param end_score		        结束score
 *
 * @returns
 *            0            succ
 *            -1            fail
 */
/* -------------------------------------------*/
int rop_zset_delrange_by_score(redisContext *conn, char* key, long start_score,long end_score)
{
    int retn = 0;
    redisReply *reply = NULL;

    //执行命令
    reply = redisCommand(conn, "zremrangebyscore %s %ld %ld", key,start_score,end_score);
